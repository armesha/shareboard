...
describe('permissionService', () => {
  describe('checkWritePermission', () => {
    ...
    it('should return false if user is undefined', () => {
      expect(checkWritePermission(workspace as Workspace, undefined)).toBe(false);
    });
    ...
    it('should handle user with accessToken = undefined', () => {
      const userWithoutToken: User = { userId: 'user-456' };
      expect(checkWritePermission(workspace as Workspace, userWithoutToken)).toBe(false);
    });
  });
  ...
  describe('checkWritePermission with sharing modes', () => {
    ...
    it('should allow owner in READ_ONLY mode', () => {
      workspace.sharingMode = SHARING_MODES.READ_ONLY as SharingMode;
      expect(checkWritePermission(workspace as Workspace, ownerUser)).toBe(true);
    });
    ...
    it('should default to READ_WRITE_SELECTED when mode is null', () => {
      workspace.sharingMode = null as unknown as SharingMode;
      expect(checkWritePermission(workspace as Workspace, regularUser)).toBe(false);
      expect(checkWritePermission(workspace as Workspace, userWithToken)).toBe(true);
    });

    it('should handle invalid sharing mode by denying access', () => {
      workspace.sharingMode = 'invalid-mode' as SharingMode;
      expect(checkWritePermission(workspace as Workspace, regularUser)).toBe(false);
      expect(checkWritePermission(workspace as Workspace, userWithToken)).toBe(false);
    });
  ...
  describe('checkOwnership', () => {
    ...
    it('should return false if userId is undefined', () => {
      expect(checkOwnership(workspace as Workspace, undefined as unknown as string)).toBe(false);
    });
    ...
    it('should return false if both workspace and userId are null', () => {
      expect(checkOwnership(null, null as unknown as string)).toBe(false);
    });
    ...
    it('should handle workspace with undefined owner', () => {
      workspace.owner = undefined as unknown as string;
      expect(checkOwnership(workspace as Workspace, 'owner-123')).toBe(false);
    });
  });
  ...
  describe('calculateEditAccess', () => {
    ...
    it('should return full access for owner regardless of token', () => {
      const result = calculateEditAccess(workspace as Workspace, ownerUser, null);
    ...
    it('should grant access with valid token', () => {
      const result = calculateEditAccess(workspace as Workspace, regularUser, 'edit_secret123');
      expect(result).toEqual({
        hasEditAccess: true,
        isOwner: false
      });
    ...
    it('should deny access with invalid token', () => {
      const result = calculateEditAccess(workspace as Workspace, regularUser, 'edit_wrongtoken');
      expect(result).toEqual({
        hasEditAccess: false,
        isOwner: false
      });
    ...
    it('should deny access without token', () => {
      const result = calculateEditAccess(workspace as Workspace, regularUser, null);
      expect(result).toEqual({
        hasEditAccess: false,
        isOwner: false
      });
    ...
    it('should return false for both flags if user is null', () => {
      const result = calculateEditAccess(workspace as Workspace, null, 'edit_token');
      expect(result).toEqual({
        hasEditAccess: false,
        isOwner: false
      });
    ...
    it('should return false for both flags if user is undefined', () => {
      const result = calculateEditAccess(workspace as Workspace, undefined, 'edit_token');
      expect(result).toEqual({
        hasEditAccess: false,
        isOwner: false
      });
    ...
    it('should handle workspace without editToken', () => {
      workspace.editToken = '';
      const result = calculateEditAccess(workspace as Workspace, regularUser, 'edit_anytoken');
      expect(result).toEqual({
        hasEditAccess: false,
        isOwner: false
      });
  ...
  describe('getSharingInfo', () => {
    ...
    it('should return correct structure for owner', () => {
      const result = getSharingInfo(workspace as Workspace, ownerUser);
      expect(result).toEqual({
    ...
    it('should return correct structure for regular user', () => {
      const result = getSharingInfo(workspace as Workspace, regularUser);
      expect(result).toEqual({
        sharingMode: SHARING_MODES.READ_WRITE_SELECTED,
        allowedUsers: ['user-456', 'user-789'],
        isOwner: false,
    ...
    it('should determine hasEditAccess correctly for allowed user', () => {
      const result = getSharingInfo(workspace as Workspace, allowedUser);
      expect(result?.hasEditAccess).toBe(true);
    });
    ...
    it('should handle null user gracefully', () => {
      const result = getSharingInfo(workspace as Workspace, null);
      expect(result).toEqual({
        sharingMode: SHARING_MODES.READ_WRITE_SELECTED,
        allowedUsers: ['user-456', 'user-789'],
        isOwner: false,
    ...
    it('should handle undefined user gracefully', () => {
      const result = getSharingInfo(workspace as Workspace, undefined);
      expect(result).toEqual({
        sharingMode: SHARING_MODES.READ_WRITE_SELECTED,
        allowedUsers: ['user-456', 'user-789'],
        isOwner: false,
  ...
  describe('validateAndSetToken', () => {
    ...
    it('should validate and grant access when token matches existing token', () => {
      ...
      const result = validateAndSetToken(workspace as Workspace, 'edit_existingtoken', user);
      expect(result).toBe(true);
      expect(user.hasEditAccess).toBe(true);
    });

    it('should not set token if workspace has no editToken (only validates existing)', () => {
      const result = validateAndSetToken(workspace as Workspace, 'edit_newtoken', user);
      expect(result).toBe(false);
      expect(workspace.editToken).toBe('');
    });

    it('should not change token when access token does not match existing', () => {
      workspace.editToken = 'edit_existingtoken';
      const result = validateAndSetToken(workspace as Workspace, 'edit_differenttoken', user);
      expect(result).toBe(false);
      expect(workspace.editToken).toBe('edit_existingtoken');
    });

    it('should reject token that does not start with "edit_"', () => {
      const result = validateAndSetToken(workspace as Workspace, 'invalidtoken', user);
      expect(result).toBe(false);
      expect(workspace.editToken).toBe('');
    });

    it('should reject empty token', () => {
      const result = validateAndSetToken(workspace as Workspace, '', user);
      expect(result).toBe(false);
      expect(workspace.editToken).toBe('');
    });
    ...
    it('should return false if accessToken is null', () => {
      const result = validateAndSetToken(workspace as Workspace, null, user);
      expect(result).toBe(false);
    });

    it('should return false if accessToken is undefined', () => {
      const result = validateAndSetToken(workspace as Workspace, undefined, user);
      expect(result).toBe(false);
    });

    it('should handle null user gracefully when validating token', () => {
      workspace.editToken = 'edit_token';
      const result = validateAndSetToken(workspace as Workspace, 'edit_token', null);
      expect(result).toBe(true);
    });

    it('should handle undefined user gracefully when validating token', () => {
      workspace.editToken = 'edit_token';
      const result = validateAndSetToken(workspace as Workspace, 'edit_token', undefined);
      expect(result).toBe(true);
    });

    it('should reject token exactly equal to "edit_" when no token set', () => {
      const result = validateAndSetToken(workspace as Workspace, 'edit_', user);
      expect(result).toBe(false);
    });
    ...
    it('should preserve other workspace properties when validating token', () => {
      const extendedWorkspace = workspace as Workspace & { otherProp?: string };
      extendedWorkspace.otherProp = 'value';
      extendedWorkspace.editToken = 'edit_token';
      validateAndSetToken(extendedWorkspace, 'edit_token', user);
      expect(extendedWorkspace.otherProp).toBe('value');
      expect(extendedWorkspace.owner).toBe('owner-123');
    });
  });
  ...
  describe('calculateEditAccess with sharing modes', () => {
    ...
    it('should default to READ_WRITE_SELECTED when mode is undefined', () => {
      workspace.sharingMode = undefined as unknown as SharingMode;
      const resultWithToken = calculateEditAccess(workspace as Workspace, regularUser, 'edit_secret123');
      expect(resultWithToken.hasEditAccess).toBe(true);

      const resultWithoutToken = calculateEditAccess(workspace as Workspace, regularUser, null);
    ...
    it('should handle empty string token', () => {
      workspace.sharingMode = SHARING_MODES.READ_WRITE_SELECTED as SharingMode;
      const result = calculateEditAccess(workspace as Workspace, regularUser, '');
      expect(result).toEqual({
        hasEditAccess: false,
        isOwner: false
      });
    ...
    it('should handle mode change from READ_WRITE_ALL to READ_ONLY', () => {
      workspace.sharingMode = SHARING_MODES.READ_WRITE_ALL as SharingMode;
      let result = calculateEditAccess(workspace as Workspace, regularUser, null);
      expect(result.hasEditAccess).toBe(true);

      workspace.sharingMode = SHARING_MODES.READ_ONLY as SharingMode;
      result = calculateEditAccess(workspace as Workspace, regularUser, null);
      expect(result.hasEditAccess).toBe(false);
    });
