...
describe('generateUserId', () => {
  it('generates unique user IDs', () => {
    ...
    expect(userId.split('-')[2]).toHaveLength(6);
  });
});
...
describe('localStorage helpers', () => {
  ...
  it('manages access tokens per workspace', () => {
    setAccessToken('ws1', 'token1');
    setAccessToken('ws2', 'token2');

    expect(getAccessToken('ws1')).toBe('token1');
    expect(getAccessToken('ws2')).toBe('token2');
    expect(getAccessToken('ws3')).toBeNull();

    removeAccessToken('ws1');
    expect(getAccessToken('ws1')).toBeNull();
    expect(getAccessToken('ws2')).toBe('token2');
  });
});
