...
export const MAX_USERS_PER_WORKSPACE = config.validation.workspace.maxUsersPerWorkspace;
...
function acquireLock(workspaceId: string, socketId: string): WorkspaceCreationLock | null {
...
async function ensureWorkspaceExists(
  ...
  const lock = acquireLock(workspaceId, socketId);
  ...
  try {
    ...
    if (socket.rooms.has(workspaceId)) {
      ...
      if (workspace) {
        ...
        return { success: true, workspace, isNewWorkspace: false, skippedDuplicate: true };
      }
    }
    ...
    const result = await ensureWorkspaceExists(workspaceId, socket.id, effectiveUserId);
