...
export function useObjectModification({
  ...
  useEffect(() => {
    ...
    const handleObjectModification = (e: { target?: ExtendedFabricObject | null }) => {
      ...
      const isActiveSelection = obj.type === 'activeselection' || obj.type === 'activeSelection';
      ...
      objectsToUpdate.forEach(item => {
        ...
        let absoluteScaleX = item.scaleX ?? 1;
        let absoluteScaleY = item.scaleY ?? 1;
        let absoluteAngle = item.angle ?? 0;
        ...
        if (isActiveSelection) {
          ...
          absoluteScaleX = Math.sqrt(matrix[0] * matrix[0] + matrix[1] * matrix[1]);
          absoluteScaleY = Math.sqrt(matrix[2] * matrix[2] + matrix[3] * matrix[3]);
          absoluteAngle = (item.angle ?? 0) + (obj.angle ?? 0);
        }

        const capturedLeft = absoluteLeft;
        const capturedTop = absoluteTop;
        const capturedScaleX = absoluteScaleX;
        const capturedScaleY = absoluteScaleY;
        const capturedAngle = absoluteAngle;
        ...
        const timeoutId = setTimeout(() => {
          ...
          try {
            updateElementByType(item, capturedLeft, capturedTop, capturedScaleX, capturedScaleY, capturedAngle, updateElement);
          } finally {
