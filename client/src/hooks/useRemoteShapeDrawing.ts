...
export function useRemoteShapeDrawing({
  ...
  const handleShapeDrawingStart = useCallback((data: ShapeDrawingStartEvent) => {
    ...
    let shape: (FabricObject & { _shapeId?: string}) | null = null;
    ...
    if (shapeType === 'line') {
      ...
      shape = new Ellipse({
        ...
        evented: false,
        objectCaching: false,
      }) as Ellipse & { _shapeId?: string};
    } else {
      shape = createShapeFromData(shapeType, shapeData) as FabricObject & { _shapeId?: string} | null;
      if (shape) {
        shape.set({
          selectable: false,
          evented: false,
    ...
    if (shape) {
      ...
      delete (shape as unknown as { id?: string }).id;
