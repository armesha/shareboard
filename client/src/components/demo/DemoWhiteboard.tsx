...
const generatePolygonPoints = (cx: number, cy: number, radius: number, sides: number, startAngle = -Math.PI / 2): string => {
  const points: Point[] = [];
...
const generateStarPoints = (cx: number, cy: number, outerRadius: number, innerRadius: number, points: number): string => {
  const result: Point[] = [];
...
const generateCrossPoints = (cx: number, cy: number, size: number): string => {
  const arm = size * 0.3;
  const half = size / 2;
  return [
    `${cx - arm},${cy - half}`,
...
interface DemoShapeProps {
  shape: Shape;
}

function DemoShape({ shape }: DemoShapeProps) {
...
const createRandomShape = (id: number, windowSize: WindowSize, gridIndex: number | null = null): Shape => {
  ...
  return {
    id,
    type: SHAPE_TYPES[Math.floor(Math.random() * SHAPE_TYPES.length)] || 'circle',
    x,
    y,
...
function DemoWhiteboard() {
  ...
  const spawnShape = useCallback(() => {
    ...
    const newShapeId = idCounter.current++;
    const newShape = createRandomShape(newShapeId, windowSize);
    ...
    requestAnimationFrame(() => {
      setShapes(prev =>
        prev.map(s => (s.id === newShapeId ? { ...s, opacity: 1 } : s))
      );
    });
  }, [windowSize]);
