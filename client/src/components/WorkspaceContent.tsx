import { useState, useEffect, useCallback, useRef, useMemo, type RefObject, type MouseEvent as ReactMouseEvent } from 'react';
...
import { MERMAID_THEME, SOCKET_EVENTS, TOOLS, CONNECTION_STATUS } from '../constants';
...
export default function WorkspaceContent({
  ...
  useEffect(() => {
    ...
    const updateViewport = () => {
      ...
      if (vpt) {
        setViewportTransform([...vpt] as ViewportTransform);
      }
    };
  ...
  const handleAddImageToWhiteboard = useCallback(async () => {
    try {
      ...
      img.onload = () => {
        ...
        const canvas = canvasRef?.current as Canvas | null;
        let centerX: number, centerY: number, scaleX: number;
