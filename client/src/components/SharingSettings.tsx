...
type CopySuccessType = '' | 'view' | 'edit';
type SharingModeType = typeof SHARING_MODES[keyof typeof SHARING_MODES];

export default function SharingSettings({ workspaceId, onClose }: SharingSettingsProps) {
  ...
  const [copySuccess, setCopySuccess] = useState<CopySuccessType>('');
  ...
  useEffect(() => {
    ...
    if (isOwner) {
      socket.emit(SOCKET_EVENTS.GET_EDIT_TOKEN, { workspaceId }, (response: EditTokenResponse) => {
  ...
  const copyToClipboard = (text: string, isEditLink = false) => {
  ...
  const handleModeChange = (e: ChangeEvent<HTMLInputElement>) => {
    changeMode(e.target.value as SharingModeType);
  };
